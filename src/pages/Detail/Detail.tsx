...
const Detail = () => {
  ...
  const { data: info, isLoading } = useFetchInfo(slug as string);
      ...
      <div className="w-full">
        <div className="w-full h-56 overflow-hidden">
          {isLoading ? (
            ...
            <Image src={info?.thumbnail!} className="w-full object-cover filter blur-lg opacity-30" />
          )}
        </div>
        <div className="space-y-4 md:space-x-4 w-full px-2 md:px-12 lg:px-24 -mt-28 md:-mt-14 flex items-center justify-center flex-col md:items-start md:justify-start md:flex-row">
          <div className="w-full md:w-44 lg:w-52 space-y-4 md:space-y-0">
            {isLoading ? (
              ...
              <Image
                src={info?.thumbnail!}
                alt={info?.name}
                className={classNames("mx-auto filter blur-none w-44 lg:w-52 h-60 lg:h-80 object-cover rounded-md md:rounded-b-none")}
              />
            )}

            {!isLoading && (
              <div className={classNames("text-white md:bg-background-darker p-3 w-full space-y-2 rounded-b-md min-h-11")}>
                <h1 className="text-base line-clamp-2">{info?.name}</h1>

                <div>
                  <h1 className="text-sm line-clamp-1">Nhóm sub: {info?.subTeams.join(", ")}</h1>
                  <h1 className="text-sm line-clamp-1">Lượt xem: {numberWithCommas(info?.views)}</h1>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="w-full px-2 flex flex-col flex-1">
            <div className="flex-col md:flex-row space-x-2 space-y-2 md:space-y-0 -mt-2 self-center md:self-start flex items-center font-bold text-lg text-white filter blur-none">
              <Button className="bg-background-darker hover:bg-opacity-80" onClick={handleClick()} startIcon={BsPlayFill} iconSize={20}>
                Xem ngay
              </Button>
              {!isLoading && storedInfo && (
                <Button className="bg-background-darker hover:bg-opacity-80" onClick={handleClick(storedInfo.episodeIndex)} startIcon={BsPlayFill} iconSize={20}>
            ...
            <div className="mt-6 space-y-2">
              <div className="space-y-1">
                ...
                {isLoading ? (
                  ...
                  <h1 className="text-gray-400 text-base">Thể loại: {info?.genres.map(({ name }) => name).join(", ")}</h1>
                )}
              </div>
