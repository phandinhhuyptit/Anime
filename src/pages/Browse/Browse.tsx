...
const BrowseScreen = () => {
  ...
  const { data, hasNextPage, isLoading, fetchNextPage, isFetchingNextPage } = useBrowseList({ category, slug: slug.join("/") });
  ...
  return (
    <div className="w-full px-2 py-20 lg:px-20 lg:py-24">
      <div className="w-full p-2">
        ...
        {isLoading && (
          <Skeleton className="my-m12 flex flex-wrap">
            {new Array(18).fill(null).map((_, i) => (
              <AnimeCardSkeleton key={i} className="mt-2 w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 2xl:w-1/6" />
            ))}
          </Skeleton>
        )}
        <div className="my-12 flex flex-wrap">
          {!isLoading &&
            list?.map((anime) => (
              <div className="mt-2 -mr-2 px-2 w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 2xl:w-1/6" key={anime.slug}>
                <AnimeCard {...anime} />
              </div>
            ))}
        </div>
