...
const ButtonRaw = (props: ButtonRawProps) => {
  ...
  return (
    <button className={classNames("flex items-center px-4 py-2 rounded-md font-medium text-xs md:text-sm lg:text-base hover:bg-opacity-80", className)} {...buttonProps}>
      {StartIcon && <StartIcon size={iconSize} className={classNames("mr-2", iconClassName)} />}
      {props.children}

      {EndIcon && <EndIcon size={iconSize} className={classNames("ml-2", iconClassName)} />}
    </button>
  );
};
