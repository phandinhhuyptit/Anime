...
import React, { HTMLProps, MutableRefObject, PropsWithChildren, useEffect, useRef, useState } from "react";
import Portal from "components/Portal";
import Overlay from "./components/Overlay";
import "./video.css";
export { addButtons } from "./components/Method/Method";
...
const Plyr: React.FC<PropsWithChildren<PlyrProps>> = (props) => {
  const { options = null, source, onReady, onSourceChange, children, nextEpisodeClick, ...rest } = props;
  ...
  const createPlayer = () => {
    ...
    plyrPlayer.on("ready", (event: PlyrJS.PlyrEvent) => {
  ...
  hls.current.on(Hls.Events.MANIFEST_LOADED, () => {
    videoOptions.quality = {
      ...
      options: hls.current.levels.map((level: Level) => level.height),
      forced: true,
      // Manage quality changes
      onChange: (quality: number) => {
        hls.current.levels.forEach((level: Level, levelIndex: number) => {
  ...
  return (
    <>
      <video ref={innerRef as unknown as MutableRefObject<HTMLVideoElement>} className="plyr-react plyr" {...rest} />
      {container && (
...
Plyr.defaultProps = {
  options: {
    ...
    controls: ["play-large", "play", "rewind", "fast-forward", "progress", "current-time", "mute", "volume", "settings", "fullscreen"],
    autoplay: true,
  },
};
...
export default React.memo(Plyr, (prevProps: PlyrProps, nextProps: PlyrProps) => {
  return prevProps.source?.sources[0].src === nextProps.source?.sources[0].src;
});
