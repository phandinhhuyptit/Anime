...
import { PlyrInstance } from "components/Video/Video";
...
const RewindButton = (props: ButtonProps) => (
  <svg aria-hidden="true" focusable="false" className="text-gray-300 hover:text-white w-8 h-8 cursor-pointer" onClick={props.onClick}>
...
const PlayButton = (props: ButtonProps & { playing: boolean }) => {
  return !props.playing ? (
    <BsFillPlayFill className="text-gray-300 hover:text-white w-16 h-16 cursor-pointer" onClick={props.onClick} />
  ) : (
    <BsPause className="cursor-pointer text-gray-300 hover:text-white w-16 h-16" onClick={props.onClick} />
  );
};
...
const PlyrControls: React.FC<PlyrControlsProps> = ({ player }) => {
  ...
  useEffect(() => {
    const defaultPlayButton = document.querySelector<HTMLElement>(".plyr__control--overlaid");
