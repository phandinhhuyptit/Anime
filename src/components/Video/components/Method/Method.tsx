...
export const addButton = (button: Button) => {
  ...
  div.className = classNames("plyr__controls__item plyr__control", `plyr_buttons--${id}`, className);

  const controlsChildNodes = controls?.childNodes;

  if (controlsChildNodes) {
    controls?.insertBefore(div, position < 0 ? controlsChildNodes[controlsChildNodes?.length! - 1] : controlsChildNodes[position - 1]);
